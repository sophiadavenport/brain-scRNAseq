...
edger_de_csv_path=f"{args.de_folder}/{args.dataset}_DEG/results/{args.dataset}_{args.DE_cellpath}__{args.celltype}_edger_results.csv"
trade_de_csv_path=f"{args.de_folder}/{args.dataset}_DEG/results/{args.dataset}_{args.DE_cellpath}__{args.celltype}_trade_sig_genes.csv"

def logfc_distribution_per_module(celltype, dataset, modules_csv_path, de_csv_path, ks_csv_path, avg_logfc_csv_path, number_dist=50, trade=True):
    ...
    if 'gene' not in de_df.columns or 'logFC' not in de_df.columns:
        if 'significant_gene_results_FDR.log2FoldChange' not in de_df.columns or 'significant_gene_results_FDR.gene' not in de_df.columns:
            print("DE file must contain 'gene' and 'logFC' columns.")
            return
        else:
            continue
    if trade==True:
        de_df['abs_logFC']=de_df['significant_gene_results_FDR.log2FoldChange'].abs()
    else:
...
if not os.path.isfile(trade_de_csv_path):
    print(f"DE TRADE file not found: {trade_de_csv_path}. Checking to see if edgeR file exists...")
elif not os.path.isfile(edger_de_csv_path):
    print(f"DE EDGER file not found: {edger_de_csv_path}. Skipping computation for {args.dataset} - {args.celltype}.")
...
if os.path.isfile(trade_de_csv_path):
    logfc_distribution_per_module(celltype=args.celltype, dataset=args.dataset, modules_csv_path=args.modules_csv_path,de_csv_path=trade_de_csv_path, 
    ks_csv_path=args.ks_csv_path, avg_logfc_csv_path=args.avg_logfc_csv_path,number_dist=args.number_dist, trade=True)
else:
    logfc_distribution_per_module(celltype=args.celltype, dataset=args.dataset, modules_csv_path=args.modules_csv_path,de_csv_path=edger_de_csv_path, 
    ks_csv_path=args.ks_csv_path, avg_logfc_csv_path=args.avg_logfc_csv_path,number_dist=args.number_dist, trade=False)